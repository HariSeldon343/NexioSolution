...
// Per i super_admin e utenti speciali, permettere upload senza azienda specifica 
if (!$aziendaId && !($auth->hasRole('super_admin') || $auth->hasRole('utente_speciale'))) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'Azienda non selezionata']);
    exit;
}

// Per super_admin e utenti speciali senza azienda, usa un valore speciale
if (!$aziendaId && ($auth->hasRole('super_admin') || $auth->hasRole('utente_speciale'))) {
    $aziendaId = 0; // Valore speciale per indicare documenti globali
}
...
try {
    ...
    // Verifica CSRF token (solo se presente nella richiesta)
