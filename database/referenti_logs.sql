...
CREATE TABLE IF NOT EXISTS referenti_aziende (
    ...
    telefono VARCHAR(20),
    ruolo_aziendale VARCHAR(100), -- es: Responsabile Qualità, Amministratore, etc.
    -- Permessi granulari
...
CREATE TABLE IF NOT EXISTS log_attivita (
    ...
    referente_id INT,
    azienda_id INT,
    tipo_entita VARCHAR(50) NOT NULL, -- 'documento', 'evento', 'utente', etc.
    id_entita INT,
    azione VARCHAR(50) NOT NULL, -- 'creazione', 'modifica', 'eliminazione', 'download', etc.
    dettagli TEXT,
    dati_precedenti JSON, -- Salva lo stato precedente per modifiche
    dati_nuovi JSON, -- Salva il nuovo stato
...
CREATE TABLE IF NOT EXISTS notifiche_email (
    ...
    tipo_notifica VARCHAR(50), -- 'documento_modificato', 'evento_creato', etc.
    azienda_id INT,
    priorita INT DEFAULT 5, -- 1-10, 1 = massima priorità
...
CREATE TABLE IF NOT EXISTS preferenze_notifiche_admin (
    ...
    tipo_notifica VARCHAR(50) NOT NULL, -- 'documento_creato', 'documento_modificato', etc.
...
-- Aggiungi constraint per limitare a 5 referenti per azienda
DELIMITER //
CREATE TRIGGER check_max_referenti
BEFORE INSERT ON referenti_aziende
FOR EACH ROW
BEGIN
    DECLARE referenti_count INT;
    SELECT COUNT(*) INTO referenti_count 
    FROM referenti_aziende 
    WHERE azienda_id = NEW.azienda_id AND attivo = TRUE;
    
    IF referenti_count >= 5 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Ogni azienda può avere massimo 5 referenti attivi';
    END IF;
END//
DELIMITER ;

-- Aggiungi colonne mancanti alla tabella aziende se non esistono
ALTER TABLE aziende 
ADD COLUMN IF NOT EXISTS richiede_conferma_azioni BOOLEAN DEFAULT TRUE;
